    ...
    public static partial class Server {
        ...
        public static bool StartServer() {
            ...
            if (listeners.Count == 0) {
                ...
                try {
                    TcpListener listener = new TcpListener(InternalIP, Port);
                    listener.Start();
            ...
            foreach (TcpListener listener in listeners) {
                ...
                if (InternalIP.Equals(IPAddress.Any)) {
                    Logger.Log(LogType.SystemActivity, "Server.Run: now accepting connections at port {0}.",
                                Port);
                } else {
                    Logger.Log(LogType.SystemActivity, "Server.Run: now accepting connections at {0}:{1}.",
                                InternalIP, Port);
                }
            }
            ...
            if( ConfigKey.AnnouncementInterval.GetInt() > 0 ) {
                // Team9000;
                TimeSpan announcementInterval = TimeSpan.FromSeconds( 45 );
        ...
        static void CheckConnections( SchedulerTask param ) {
            List<TcpListener> listenersCache = listeners;
            foreach (TcpListener listener in listenersCache) {
                if (listener.Pending()) {
                    try {
                        Player.StartSession(listener.AcceptTcpClient());
                    } catch (Exception ex) {
                        Logger.Log(LogType.Error,
